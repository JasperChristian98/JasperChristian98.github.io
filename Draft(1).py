...
def latest_team_data(manager):
    ...
    for gw in reversed(finished_gws):
        teams = history["gameweeks"][str(gw)].get("teams", {})
...
def manager_team_card(manager, card_index):
    gw, team_data = latest_team_data(manager)
    if not team_data:
            ...
            <div class="my-team-grid">
                <div>
                    ...
                    <div class="stats-grid compact-stats">
                        <div class="stat-card"><div class="stat-label">Latest GW</div><div class="stat-value">{team_data.get("gw_points", 0)}</div><div class="stat-description">Team points</div></div>
...
def gameweek_summary_sections():
    ...
    for index, gw in enumerate(finished_gws):
        ...
        for team in gw_data.values():
            try:
                scores.append((team.get("manager", "Unknown"), int(team.get("gw_points", 0) or 0)))
            except (TypeError, ValueError):
                pass
...
# NOTE
# ============================================================
# The dashboard is written to index.html above.
# Interactive notebook display is intentionally omitted so this
# script can run cleanly in GitHub Actions / a headless runner.
